...
from .networks import MultiHeadSelfAttention


class PatchDiscriminatorMultiheadSelfAttentionModel(BaseModel):

    def name(self):
        return 'PatchDiscriminatorMultiheadSelfAttentionModel'
    ...
    def __init__(self, opt):
        ...
        tmp = MultiHeadSelfAttention.MultiHeadSelfAttention(
            dim=256, heads=1, dim_head=32)
        self.net_M = netutils.init_net(
            tmp, init_type=None, gpu_ids=self.gpu_ids)

        tmp = nn.Conv2d(256, 4, kernel_size=1)
        self.net_K = netutils.init_net(tmp, opt.init_type, self.gpu_ids)

        tmp = nn.Conv2d(6, 2, kernel_size=1)
        self.net_L = netutils.init_net(tmp, opt.init_type, self.gpu_ids)

        if self.isTrain:
            self.params = list(self.net_D.parameters())
            self.optimizers['D'] = torch.optim.Adam(
                self.net_D.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.params = list(self.net_M.parameters())
            self.optimizers['M'] = torch.optim.Adam(
                self.params, lr=opt.lr, betas=(opt.beta1, 0.999))

            self.params = list(self.net_K.parameters())
            self.optimizers['K'] = torch.optim.Adam(
                self.params, lr=opt.lr, betas=(opt.beta1, 0.999))

            self.params = list(self.net_L.parameters())
    ...
    def forward(self):
        # N = batch size
        # lay0: patch classifier output after pass through 1x1 conv
        # lay1: patch classifier output before pass through 1x1 conv
        lay0, lay1 = self.net_D(self.ims)

        # lay2: attention layer output
        N, C, H, W = lay1.shape
        lay1 = lay1.reshape(N, C, H*W)
        lay1 = lay1.transpose(1, 2)
        lay2 = self.net_M(lay1)
        lay2 = lay2.transpose(1, 2).reshape(N, C, H, W)

        # net_M_out: shrink lay2 to 4 channels
        net_M_out = self.net_K(lay2)

        concat = torch.cat((lay0, net_M_out), dim=1)

        # self.pred_logit: final output (2 x 37 x 37)
        self.pred_logit = self.net_L(concat)

    def forward_vis(self, img):
        # N = batch size
        # lay0: patch classifier output after pass through 1x1 conv
        # lay1: patch classifier output before pass through 1x1 conv
        lay0, lay1 = self.net_D(img)
        ...
        # self.pred_logit: final output (2 x 37 x 37)
        pred_logit = self.net_L(concat)

        return pred_logit, lay0
    ...
    def visualize(self, pred_outputs, pred_paths, labels, transform,
        ...
        for indices, desc in zip([order[:n], order[-n:][::-1]], ['hardest', 'easiest']):
            ...
            for c, i in enumerate(indices, 1):
                ...
                color = (0, 255, 0) if predicted_class == target_label else (
                    255, 0, 0)
                ...
                for label in [1]:  # just plot P(real)
                    ...
                    result.save(os.path.join(dirname, desc,
                                '%03d_patch_%d.png' % (c, label)))
