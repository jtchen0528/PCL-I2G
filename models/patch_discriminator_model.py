...
class PatchDiscriminatorModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if net_D_blocknum == 1:
            net_D_outch = 128
        elif net_D_blocknum == 2:
            net_D_outch = 256
        elif net_D_blocknum == 3:
            net_D_outch = 728
        elif net_D_blocknum == 5:
            net_D_outch = 728
        ...
        # # load reproduced pre-trained
        # self.state_dict_pth = '/scratch2/users/jtchen0528/patch-forensics-cat/checkpoints/new-block12-DF-10epoch-pretrain-more/10_net_D.pth'
        # tmp = torch.load(self.state_dict_pth, map_location = torch.device('cuda', self.gpu_ids[0]))
        # self.net_D.module.load_state_dict(tmp['state_dict'])

        # self.state_dict_pth = '/scratch2/users/jtchen0528/patch-forensics-cat/checkpoints/new-block12-DF-10epoch-pretrain-more/10_net_D_output.pth'
        # tmp = torch.load(self.state_dict_pth, map_location = torch.device('cuda', self.gpu_ids[0]))
        # self.net_D_output.module.load_state_dict(tmp['state_dict'])

        if self.isTrain:
            self.optimizers['D'] = torch.optim.Adam(
                self.net_D.parameters(), lr=opt.lr*0.01, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        self.pred_logit = self.net_D_output(self.net_D(self.ims))
