...
def make_patch_resnet(depth, layername, num_classes=2, extra_output=None):
    def change_out(layers):
        ...
        if layername.startswith('layer'):
            bn = list(layer.modules())[-1 if depth <
                                       50 else -2]  # find final batchnorm
            assert(isinstance(bn, nn.BatchNorm2d))
            num_ch = bn.num_features
        else:
            num_ch = 64
        layers[ind+1:] = [('convout', nn.Conv2d(num_ch,
...
def make_patch_xceptionnet(layername, num_classes=2, extra_output=None):
    ...
    if (extra_output == None):
        ...
        model = CustomXceptionNet(
            extra_output=extra_output, modify_sequence=change_out)
    return model


def make_pcl(backbone='xception', layername='block3', input_size=128):
    if backbone == 'xception':
        channels = [128, 256, 728, 728, 728, 728, 728, 728, 728,
                    728, 728, 1024]
        b1, b2, b3, b12 = int(input_size / 4), int(input_size /
                                                   8), int(input_size / 16), int(input_size / 32)
        ...
        b1, b2, b3, b4 = int(input_size / 2), int(input_size /
                                            4), int(input_size / 8), int(input_size / 16)
        out_ch = [b1, b2, b3, b4]
        channel = out_ch[layer - 1]
    from . import PCL
...
class CustomXceptionNet(nn.Module):
    ...
    def __init__(self, channels=None, num_classes=1000,
        ...
        sequence = modify_sequence([
            ...
            # does adaptive_avg_pool and flatten
            ('avgpool', GlobalAveragePool2d()),
